...
int index_main(int argc, char **argv, char *full_cmd) {
    ...
    int c;
    char *input_pattern = NULL, *output_dir_name = NULL, *metadata_conf_name = NULL;
    char *i_type = "i";
     ...
     *
     * Assumptions:
     * 1. giggle input_dir_name is a glob pattern that is only in a single directory
     * 2. giggle output_dir_name and input_dir_name have the same parent
     *     - we could technically let the input_dir_name glob pattern cover several
     *       directories as long as their parent is the same as the output_dir_name,
     *       but for now it's easiest to just require the input_dir_name to be a single
     *       directory.
     *
     * Procedure:
     * 1. Get the absolute path for one of the input files (using glob)
     * 2. Get the parent directory (basename of dirname)
     * 3. chdir to the parent directory so that everything is relative to that directory
     * 4. Pass the $data_dir/$glob_pattern to giggle indexing functions where $data_dir
     *    is the basename(dirname) of input_dir_name and $glob_pattern is basename of
     *    input_dir_name eg. if input_dir_name is "/path/to/data/\*.gz", then
     *    data_dir = "data".
     */
